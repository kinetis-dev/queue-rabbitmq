...
//
// basic.get is always immediate, so pop() sweeps every queue in
// priority order before it waits at all. This confirms a job in the
// last of several empty higher-priority queues is still found quickly,
// and that every named queue gets declared (a real AMQP operation)
// along the way.
$start = microtime(true);
